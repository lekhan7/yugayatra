...
const AnimatedDashboardContent = () => {
  ...
  return (
    <>
      ...
      {/* Render components based on visibility and order */}
      {settings.layout.componentOrder.map((componentName) => {
        if (!settings.layout.componentVisibility[componentName]) return null
        
        switch (componentName) {
          case 'stats':
            return (
              <motion.div key="stats" variants={itemVariants} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {statCards.map((stat, index) => (
                  <motion.div
                    ...
                    style={{
                      backgroundColor: settings.theme.backgroundGradient ? undefined : 'white',
                      boxShadow: settings.theme.cardShadows ? undefined : 'none'
                    }}
                  >
                    <div className={`absolute inset-0 bg-gradient-to-br ${stat.color} opacity-0 group-hover:opacity-10 transition-opacity duration-300`} />
                    <div className="relative">
                      <div className="flex items-center justify-between mb-4">
                        {settings.components.stats.showIcons && (
                          <div className={`p-3 rounded-xl bg-gradient-to-br ${stat.color} text-white shadow-lg`}>
                            <stat.icon size={20} />
                          </div>
                        )}
                        {settings.components.stats.showTrends && (
          ...
          case 'charts':
            return (
              <motion.div key="charts" variants={itemVariants} className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {/* Application Trends */}
                <motion.div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6">
                  <div className="flex items-center justify-between mb-6">
                    <h2 className="text-xl font-bold text-gray-900 dark:text-white">Application Trends</h2>
                    <Activity className="text-blue-500" size={20} />
                  </div>
                  <ResponsiveContainer width="100%" height={settings.components.charts.chartHeight}>
                    <AreaChart data={applicationTrends}>
                      <defs>
                        <linearGradient id="colorApplications" x1="0" y1="0" x2="0" y2="1">
                          <stop offset="5%" stopColor={settings.theme.primaryColor} stopOpacity={0.8}/>
                          <stop offset="95%" stopColor={settings.theme.primaryColor} stopOpacity={0.1}/>
                        </linearGradient>
                      </defs>
                      {settings.components.charts.showGrid && <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />}
                      <XAxis 
                        dataKey="date" 
                        stroke="#6b7280"
                        fontSize={12}
                      ...
                      <Area 
                        ...
                        stroke={settings.theme.primaryColor} 
                        fillOpacity={1} 
                  ...
                  <ResponsiveContainer width="100%" height={settings.components.charts.chartHeight}>
                    <RePieChart>
                      ...
                      <Tooltip 
                        ...
                        labelStyle={{ color: '#1f2937', fontWeight: '600' }}
                      />
                      {settings.components.charts.showLegend && <Legend />}
                    </RePieChart>
                  </ResponsiveContainer>
                </motion.div>
              </motion.div>
          ...
          case 'monthly':
            return (
              <motion.div key="monthly" variants={itemVariants} className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6">
                <div className="flex items-center justify-between mb-6">
          ...
          case 'activity':
            return (
              <motion.div key="activity" variants={itemVariants} className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6">
                <div className="flex items-center justify-between mb-6">
                ...
                <div className="space-y-3">
                  <AnimatePresence>
                    {recentActivity.slice(0, settings.components.activity.maxItems).map((activity, index) => {
                        ...
                        >
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-xs text-gray-500 dark:text-gray-400">
                              {activity.subtitle} {settings.components.activity.showTimestamps && `• ${activity.time}`}
                            </p>
                          </div>
                        </motion.div>
                      )
